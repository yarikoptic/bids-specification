...
def validate_all(
    paths,
    regex_schema,
    pseudofile_suffixes=[],  # NOT USED?! remove/deprecate?
):
    """
    ...
    ----------
    paths : list of str
        Paths which should be validated.
    regex_schema : list of dict
    ...
    * Currently only entities are captured in named groups, edit `load_top_level()` to name other
    ...
    results = {
        "itemwise": itemwise_results,
        "schema_tracking": tracking_schema,
        "schema_listing": regex_schema,
        "path_tracking": tracking_paths,
        "path_listing": paths,
...
def validate_bids(
    ...
    >>> bids_dataset = '~/.data2/datalad/000026/rawdata'
    >>> schema_version='{module_path}/data/schema/'
    >>> validator.validate_bids(bids_dataset, schema_version=schema_version)"
